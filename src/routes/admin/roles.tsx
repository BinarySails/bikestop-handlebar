...
function RolesPage() {
  ...
  const allRoles = useMemo(
    () => (data?.data?.roles ?? []).filter((r) => r.status !== "archive"),
    [data?.data?.roles]
  );
  ...
  const filteredRoles = useMemo(() => {
    ...
    return allRoles.filter((role) => {
      if (statusFilter === "active" && role.status !== "enable") return false;
      if (statusFilter === "inactive" && role.status !== "disable")
        return false;
      if (query) {
        const matches =
  ...
  const columns: EntityColumn<Role>[] = [
    ...
    },
    {
      ...
      cell: (role) => (
        <Badge
          variant={role.status === "enable" ? "default" : "secondary"}
          className="gap-1"
        >
          {role.status === "enable" ? (
